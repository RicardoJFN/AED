...
public class TestLinkedList {
  public static void main(String args[]) {
   /*LinkedList<Integer> l = new LinkedList<Integer>();
    ...
    LibLinkedList.addLast(newL, new Integer(arr[i]));
      //LibLinkedList.addFirst(newL, new Integer(arr[i]));


    LibLinkedList.filter(newL, 3,8);

    char[] arrChar = new char[] {'a', 'b', 'c', 'd', 'e', 'f'};
    LinkedList<Character> newC = new LinkedList<>();
    for (int i = 0; i != arrChar.length; i++){
      LibLinkedList.addLast(newC, new Character(arrChar[i]));
    }
    LibLinkedList.filter(newC, 'b', 'd');
  }
}
